...
const healthSchema = z.object({
  health_record_type: z.string().min(1, 'Record type is required'),
...
export default function HealthForm({ animal, date, userInitials, existingLog, onSave, onCancel }: HealthFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);

  const form = useForm({
    defaultValues: {
      health_record_type: existingLog?.health_record_type || '',
      notes: existingLog?.notes || ''
    },
    onSubmit: async ({ value }) => {
      setIsSubmitting(true);
      try {
        const safePayload = healthSchema.parse(value);
        const payload: Partial<LogEntry> = {
          ...
          health_record_type: safePayload.health_record_type,
          value: safePayload.health_record_type,
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); form.handleSubmit(); }} className="space-y-6">
      <form.Field name="health_record_type" children={(field) => (
        <div>
          <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Record Type</label>
          <input type="text" value={field.state.value} onChange={e => field.handleChange(e.target.value)} className="w-full p-3 bg-slate-50 border-2 border-slate-200 rounded-xl" required />
        </div>
      )} />
