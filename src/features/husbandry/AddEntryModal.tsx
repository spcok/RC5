...
const AddEntryModal: React.FC<AddEntryModalProps> = ({ 
  ...
  const [userInitials] = useState(currentUser?.initials || '');
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          <div className="space-y-2">
            ...
            {/* 
              AUTO-FILLED: Retrieved from authStore. 
              Read-only to ensure audit trail integrity.
            */}
            <input 
              type="text" 
              value={String(userInitials || '')} 
              readOnly 
              className="w-full p-3 bg-slate-50 border-2 border-slate-200 rounded-xl font-bold text-xs text-slate-500 cursor-not-allowed" 
              placeholder="e.g. JD" 
              required 
            />
          </div>
